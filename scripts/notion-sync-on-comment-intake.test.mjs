// Tests for scripts/notion-sync-on-comment-intake.mjs
// Run with: node --test
import { test } from "node:test";
import assert from "node:assert/strict";
import { run } from "./notion-sync-on-comment-intake.mjs";
...
test('a comment on an "In GitHub" row sets 対応Status=Fixed and GitHub Status=Closed', async () => {
...
test('does NOT act when the row is not "In GitHub" (e.g. still Intake)', async () => {
  const { fetchImpl, calls } = makeFakeFetch([
    { match: isQuery, respond: () => queryResult([pageWithStatus("p2", "Intake")]) },
  ]);

  const result = await run({ env: env(), fetchImpl, logger: silent });

  assert.deepEqual(result, { matched: 1, updated: 0, skipped: 1 });
  assert.ok(!calls.some((c) => isPatch(c.url, c.options)), "must not PATCH a non-In-GitHub row");
});

test('does NOT drag a row backward from a later status (e.g. Fixed)', async () => {
  const { fetchImpl, calls } = makeFakeFetch([
    { match: isQuery, respond: () => queryResult([pageWithStatus("p3", "Fixed")]) },
  ]);
